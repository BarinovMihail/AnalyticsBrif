...
class SupplierEntry(Base):
    ...
    okpd2_code: Mapped[str | None] = mapped_column(String(255))
    mtr_class: Mapped[str | None] = mapped_column(String(255))
    supplier_site: Mapped[str | None] = mapped_column(String(255))
    manufacturer_inn: Mapped[str | None] = mapped_column(String(255), index=True)
...
class MTRCard(Base):
    ...
    mtr_class: Mapped[str | None] = mapped_column(String(255))
    price: Mapped[float | None] = mapped_column(DECIMAL(15, 2))
    currency_code: Mapped[str | None] = mapped_column(String(255))
    delivery_date: Mapped[datetime | None] = mapped_column(Date)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow, nullable=False)
